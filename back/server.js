...
const db = pgp("postgres://artemdb:metra2856030@data_base:5432/data_gav_copy");//192.168.237.128//data_base
...
app.post('/api/data/sensors', (req, res) => {
  ...
  if (req.body.id.length === 2 && req.body.lastHours) {
    db.any(`
      ...
      FROM sdata s1
      LEFT OUTER JOIN sdata s2 ON date_trunc('minute', s1.date_time) = date_trunc('minute', s2.date_time)
    ...
    db.any(`
      ...
      LEFT OUTER JOIN sdata s2 ON date_trunc('minute', s1.date_time) = date_trunc('minute', s2.date_time)
      AND s2.smcpc_id = ${req.body.id[1]}
      LEFT OUTER JOIN sdata s3 ON date_trunc('minute', s1.date_time) = date_trunc('minute', s3.date_time)
    ...
    db.any(`
      ...
      LEFT OUTER JOIN sdata s3 ON date_trunc('minute', s1.date_time) = date_trunc('minute', s3.date_time)
      AND s3.smcpc_id = ${req.body.id[2]}
      LEFT OUTER JOIN sdata s4 ON date_trunc('minute', s1.date_time) = date_trunc('minute', s4.date_time) 
